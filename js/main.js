...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createStaticTransition() {
    ...
    div.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: var(--bg-base); z-index: 10000; opacity: 0; pointer-events: none; transition: opacity 0.3s ease;';
    document.body.appendChild(div);
    
    const style = document.createElement('style');
    style.textContent = '.static-transition.active { opacity: 1; pointer-events: all; }';
  ...
  if (vinylPlayer && ambientNoise) {
    vinylPlayer.addEventListener('click', function() {
      if (isPlaying) {
        ambientNoise.pause();
...
(function() {
  ...
  window.openLightbox = function(imageSrc, caption, images) {
    images = images || [];
  ...
  if (lightboxEl) {
    lightboxEl.addEventListener('click', function(e) {
      if (e.target === lightboxEl || e.target.classList.contains('lightbox-overlay')) {
...
(function() {
  ...
  // 开屏动画完成后启用自定义鼠标
  document.addEventListener('openingComplete', function() {
    if (cursor) {
